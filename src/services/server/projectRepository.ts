...
export async function listAllProjects() {
  try {
    const result = await dynamo.send(new ScanCommand({ TableName: tables.projects }));
    return (result.Items ?? []) as Project[];
  } catch {
    return demoProjects;
  }
...
export async function getProjectById(id: string) {
  const result = await dynamo.send(new GetCommand({ TableName: tables.projects, Key: { id } }));
